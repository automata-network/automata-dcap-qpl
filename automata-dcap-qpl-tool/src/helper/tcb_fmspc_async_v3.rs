//! Offchain driver for FmspcTcbDaoV3 — closes the V2 step-2/step-3 consistency hole by
...
//! `automata-on-chain-pccs/src/bases/FmspcTcbDaoV3.sol` and
//! `automata-on-chain-pccs/src/helpers/FmspcTcbHelperV3.sol`.

use super::tcb_fmspc_async::{
    extract_tcb_info_str, fallback_tcb_version, generate_ref_id, parse_tcb_type, send_transaction, TcbLocator,
};
...
const DEFAULT_V3_PARSE_BATCH_SIZE: u64 = 3;
const V3_PARSE_BATCH_SIZE_ENV: &str = "QPL_ASYNC_PARSE_BATCH_SIZE";

fn async_parse_batch_size_v3() -> u64 {
    std::env::var(V3_PARSE_BATCH_SIZE_ENV)
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|n| *n > 0)
        .unwrap_or(DEFAULT_V3_PARSE_BATCH_SIZE)
}
...
abigen!(
    ...
    r#"[
        ...
        function commitBasicsV3(bytes32 refId)
        ...
        function uploadParsedTcbLevelsBatchV3(bytes32 refId, uint256 start, uint256 itemCount, bytes batchStream) returns (uint256 parsed, uint256 total, bool complete)
        function uploadParsedTdxModuleIdentitiesBatchV3(bytes32 refId, uint256 start, uint256 itemCount, bytes batchStream) returns (uint256 parsed, uint256 total, bool complete)
        function finalizeAsyncUpsert(bytes32 attestationId, bytes32 refId) returns (bytes32)
    ]"#
);

abigen!(
    CollateralResolverV3,
    r#"[
        function collateralPointer(bytes32 key) view returns (bytes32)
    ]"#
);

pub async fn upsert_tcb_fmspc_v3_func(
    private_key: String,
    rpc_url: String,
    chain_id: u64,
    gas_price: U256,
    ...
    let dao = FmspcTcbDaoV3::new(parse_address_from_str(fmspc_tcb_dao_contract_addr), signer.clone());

    run_upsert(&dao, signer.as_ref(), &log_prefix, gas_price, &raw_inner, &tcb_info_obj, locator).await
}

/// Run the 5-stage V3 upsert end-to-end.
    ...
    M::Error: 'static,
{
    ...
    // Three staged commits (vs the bundled `commitBasicsV3`) keep each tx under the per-block
    ...
    // Each V3 upload-batch tx does per-level reverse-serialization + keccak round-trip against
    ...
    let batch_size = async_parse_batch_size_v3() as usize;
    let mut sent = 0usize;
    while sent < levels.len() {
        ...
        if !send_transaction(
            signer,
            dao.upload_parsed_tcb_levels_batch_v3(
                ref_id,
                U256::from(sent),
                U256::from(end - sent),
                chunk_stream.into(),
            ...
            .gas_price(gas_price),
            log_prefix,
            "upload_parsed_tcb_levels_batch_v3",
        )
        .await
        {
            return false;
    ...
    if !identities.is_empty() {
        ...
        while sent < identities.len() {
            ...
            if !send_transaction(
                signer,
                dao.upload_parsed_tdx_module_identities_batch_v3(
                    ref_id,
                    U256::from(sent),
                    U256::from(end - sent),
                    chunk_stream.into(),
                ...
                .gas_price(gas_price),
                log_prefix,
                "upload_parsed_tdx_module_identities_batch_v3",
            )
            .await
            {
                return false;
    ...
    let resolver = CollateralResolverV3::new(resolver_addr, signer.into());
...
fn load_payload_from_file(
    ...
    let payload = if trimmed.starts_with(r#"{"tcbInfo":{"#) && trimmed.contains(r#","signature":"#) {
        ...
        let sig = signature_hex
            .ok_or_else(|| "missing signature hex; pass via --tcb_info_signature_hex".to_string())?;
        let sig = sig.trim().trim_start_matches("0x");
