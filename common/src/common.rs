...
impl SgxQlConfig {
    pub fn new(cpu_svn: [u8; 16], isv_svn: u16, mut cert: Vec<u8>) -> Self {
...
impl SgxQlQveCollateral {
    pub fn new(
        ...
        tee_type: u32,
        mut pck_crl_issuer_chain: Vec<c_char>,
        mut root_ca_crl: Vec<c_char>,
        mut pck_crl: Vec<c_char>,
        mut tcb_info_issuer_chain: Vec<c_char>,
        mut tcb_info: Vec<c_char>,
        mut qe_identity_issuer_chain: Vec<c_char>,
        mut qe_identity: Vec<c_char>,
    ) -> Self {
        let pck_crl_issuer_chain_size = pck_crl_issuer_chain.len();
        let pck_crl_issuer_chain = pck_crl_issuer_chain.as_mut_ptr();
        let root_ca_crl_size = root_ca_crl.len();
        let root_ca_crl = root_ca_crl.as_mut_ptr();
        let pck_crl_size = pck_crl.len();
        let pck_crl = pck_crl.as_mut_ptr();
        let tcb_info_issuer_chain_size = tcb_info_issuer_chain.len();
        let tcb_info_issuer_chain = tcb_info_issuer_chain.as_mut_ptr();
        let tcb_info_size = tcb_info.len();
        let tcb_info = tcb_info.as_mut_ptr();
        let qe_identity_issuer_chain_size = qe_identity_issuer_chain.len();
        let qe_identity_issuer_chain = qe_identity_issuer_chain.as_mut_ptr();
        let qe_identity_size = qe_identity.len();
        let qe_identity = qe_identity.as_mut_ptr();
